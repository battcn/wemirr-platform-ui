  ...
  <el-card style="margin-bottom:20px;">
    ...
    <div class="user-profile">
      <div class="box-center">
        <pan-thumb :height="'110px'" :hoverable="false" :image="avatar" :width="'110px'">
      ...
      <div class="box-center">
        ...
        <div class="user-role text-center text-muted">
          <span>{{ user.email ? user.email : '暂无邮箱' }}</span> ·
          <span>{{ user.mobile ? user.mobile : '暂无手机' }}</span>
        </div>
      </div>
    </div>
    <div class="user-bio">
      <div class="user-education user-bio-section">
        ...
        <div class="user-bio-section-body">
          <div class="text-muted">{{ user.workDescribe ? user.workDescribe : '暂无描述' }}</div>
        </div>
      </div>
      <div class="user-education user-bio-section">
        <div class="user-bio-section-header">
          ...
          <div class="text-muted">
            <template v-for="(l, index) in logo">
              <div :key="index" class="logo-wrapper">
                <img :class="{ 'radius': l.radius }" :src="resolveLogo(l.img)" title="未绑定"
                     @click="unbind(l.name)" alt v-if="l.bind"/>
                <img :class="{ 'radius': l.radius }" :src="resolveLogo(l.img)" title="绑定"
                     @click="bind(l.name)" alt class="unbind" v-else/>
              </div>
            </template>
          </div>
        </div>
