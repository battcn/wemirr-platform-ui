        ...
        <el-form ref="form">
          <el-row v-for="(val, index) in dataItem" :key="index" style="margin-bottom: 10px;">
            ...
            <el-col style="width:180px;margin-left: 10px;">
              <dict-select clearable v-model="dataItem[index].expression" :disabled="dataItem[index].column === null" :dict="getDict({
                  data: [
                    {'value':'eq',label:'包含（ = ）'},
                    {'value':'ne',label:'不等于（ <> ）'},
                    {'value':'gt',label:'大于（ > ）'},
            ...
            <el-button type="primary" icon="el-icon-plus" plain style="margin-left:10px;" circle @click="add" v-if="index === 0"/>
