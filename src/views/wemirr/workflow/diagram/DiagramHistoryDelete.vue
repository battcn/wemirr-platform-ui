...
<script lang="ts" setup>
  ...
  import { deleteByModel } from '/@/api/modules/process/base/designModelHistory';
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    modelInfo.historyModelId = data?.historyModelId || '';
    setFieldsValue({
      ...data,
    });
  });
  ...
  async function handleSubmit() {
    try {
      ...
      values.historyModelId = modelInfo.historyModelId;
