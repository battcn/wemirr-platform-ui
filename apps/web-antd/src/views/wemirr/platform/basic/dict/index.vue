...
const loadDictList = async () => {
  treeData.value = await api.GetList();
};
...
const handleDelete = (node: any) => {
  Modal.confirm({
    ...
    onOk: async () => {
      ...
      await loadDictList();
      ui.notification.success({ message: '删除成功' });
    },
  });
};
...
const refreshDictCache = async () => {
  await api.Refresh();
  ui.notification.success({ message: '字典缓存刷新成功' });
};
...
const handleMenuMouseEnter = (key) => (hoveredKey.value = key);
const handleMenuMouseLeave = () => (hoveredKey.value = '');
const handleSearch = () => {};
</script>
