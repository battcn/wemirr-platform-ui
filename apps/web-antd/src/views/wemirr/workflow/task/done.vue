...
import { currentUserDonePage } from '#/api/workflow/task';
...
async function reload(resetFields: boolean = false) {
  ...
  loading.value = true;
  const resp = await currentUserDonePage({
    size: 10,
    current: page.value,
    ...formData.value,
  });
...
async function handleCardClick(item: TaskInfo) {
  ...
  // 点击的是同一个
  if (lastSelectId.value === instanceId) {
    return;
  }
  currentTask.value = item;
  // 反选状态 & 如果已经点击了 不变 & 保持只能有一个选中
  taskList.value.forEach((item) => {
    item.active = item.instanceId === instanceId;
  });
  lastSelectId.value = instanceId;
}
