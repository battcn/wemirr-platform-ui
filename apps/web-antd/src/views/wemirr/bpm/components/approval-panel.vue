...
function handleTermination() {
  approveWithReasonModal({
    ...
    onOk: async (reason) => {
      await terminationTask({ taskId: props.task!.id, message: reason });
      emit('reload');
    },
  });
}
    ...
    >
      <template #title>
        <div class="flex items-center gap-2">
          <div>流程编号: {{ task.instanceId }}</div>
          <CopyOutlined
            class="cursor-pointer"
            @click="handleCopy(task.instanceId)"
          />
        </div>
      </template>
      <template #extra>
        ...
        <div class="flex flex-col gap-3">
          <div class="flex items-center gap-2">
            ...
            <div>
              <component
                :is="
                  renderLocalDict(task.flowStatus, LocalDictCode.WF_TASK_STATUS)
                "
              />
            </div>
          </div>
